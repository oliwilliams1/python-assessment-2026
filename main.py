...
class Restaurant:
    ...
    def modify_order(self, menu_item: MenuItem, delta: int):
        # Apply the delta to the current quantity, ensuring it doesn't go below 0
    ...
    def update_past_order_summary(self):
        ...
        for i, order in enumerate(self.past_orders):
            # If this order isn't empty, then at least one order is vald
    ...
    def layout_menu_item(self, parent: ctk.CTkFrame, menu_item: MenuItem):
        ...
        # Decrement button, quantity label, and increment button with reactive commands to update the order and GUI when clicked
